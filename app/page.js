...
<div className="z-10 flex" style={{ background: CARD, borderBottom: `1px solid ${LINE}`, fontSize: `${16 * FONT_SCALES[fontScale] * 0.7}px` }}>
        <div className="flex items-center pl-5 sm:pl-8 flex-shrink-0">
          <LogoMark size={40} />
                  <span style={{ fontFamily: DISPLAY_FONT, fontSize: `${24 * FONT_SCALES[fontScale] * 0.7}px`, color: INK, lineHeight: 1 }} className="ml-2.5">장편</span>
        </div>
        ...
        <div className="flex items-center justify-end px-5 sm:px-8 py-3.5 relative">
          ...
          <span style={{ fontSize: `${10 * FONT_SCALES[fontScale] * 0.7}px`, fontWeight: 700, color: INK_SOFT }} className="mr-0.5">글자크기</span>
        ...
        {NAV.map((n) => {
          ...
          return (
            <button key={n.id} onClick={() => setTab(n.id)} className="flex flex-col items-center justify-center gap-0.5 flex-1 py-1.5 transition-all duration-200 active:scale-95"
              ...
              <Icon size={17} />
               <span style={{ fontSize: `${10 * FONT_SCALES[fontScale] * 0.7}px`, fontWeight: 700 }}>{n.label}</span>
            </button>
          );
        })}
      </div>
